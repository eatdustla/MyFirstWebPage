...
document.addEventListener('DOMContentLoaded', function () {
    const popupToggleFactors = document.getElementById('popup-toggle-factors');
    const popupToggleExtend = document.getElementById('popup-toggle-extend');
    const popupFactors = document.querySelector('.popup');
    const popupExtend = document.querySelector('.popup-extend');
  
    popupToggleFactors.addEventListener('change', togglePopup.bind(null, popupFactors));
    popupToggleExtend.addEventListener('change', togglePopup.bind(null, popupExtend));
    ...
    function closePopupOnClickOutside(event) {
      ...
      if (event.target !== popupFactors && event.target !== popupExtend) {
        popupFactors.style.right = '-100%';
        popupExtend.style.right = '-100%';
      }
    }
  });
